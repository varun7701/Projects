...
#include "stdio.h"
#include "string.h"
#include "../usb_kb1/project_config.h"
...
BYTE SPI_wr(BYTE data) {
	...
	//return the status register
		int return_code;
	    BYTE read_data;
		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &data,
											 1, &read_data, 0);
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int return_code;
		BYTE read_data;
		BYTE notptr = reg + 2;
		BYTE combined[2] = {notptr, val};
		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, combined,
												 0, &read_data, 0);

	//		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, val, 0, 0, 0);
		if(return_code < 0){
			alt_printf("ERROR SPI TX RET = %x \n", return_code);
		}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
		...
		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 9, input,
													 0, &read_data , 0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int return_code;
		BYTE val;
		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg,
													 1, &val, 0);
